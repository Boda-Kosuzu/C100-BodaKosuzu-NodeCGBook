...
それでは、最後にNodeCGを起動してみましょう。
ページにアクセスすると、package.jsonファイルのdashboardPanels配下のtitleで指定した名前のパネルが表示されているかと思います。

また、右上のGRAPHICSを開くとpackage.jsonファイルの全体のtitleとして指定したパネルの中に、index.HTMLが表示されていると思います。
このindex.HTMLはgraphics配下のfileで指定した者になっています。

右上のASSETSを開くとGRAPHICSと同様にtitleとして指定したパネルがあり、その中にassetCategoriesのtitleで指定したタイトルが表示されていると思います。
ここのADD FILESからファイルを追加することで、このバンドル内で共有して使うことができます。
