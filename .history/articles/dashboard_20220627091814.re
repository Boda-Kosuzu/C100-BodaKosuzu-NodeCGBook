...
()の中にはどの画面とやり取りをするかを指定します。
今回の場合は@<code>{assets:nodecg-werewolf-games}となっていますが、これはAssets内のnodecg-werewolf-gamesというnameを指定しています。
これ前章で作成したpackage.jsonで指定したnameと同じ必要があります。
