...
プログラミングは基本的に3つの概念で成り立っているといえます。
それは変数、条件分岐、ループ
の3つです。
...
JavaScriptはWebページに根付いた言語であるため、ブラウザウィンドウの操作や、表示されているHTMLであるDOM要素を操作する機能があります。
ウィンドウの操作は@<code>{window.hoge}、DOM要素の操作は@<code>{document.hoge}といった形で用意されています。
ただし、NodeCGで主に使うものは@<code>{document}で用意されているものなため、@<code>{window}は忘れても構いません。
