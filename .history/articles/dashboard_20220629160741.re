...
//}
@<list>{js4}、@<list>{js5}、@<list>{js6}がそれぞれ画像ファイル名を表示するDOM、役職を選択するDOM、生死状態を制御するDOMを作成して、関数の呼び出しもとに返す関数です。

@<list>{js4}から見ていきましょう。
これは気ほども利用したcreateElementで作成し、そこにinnerTextで受け取った名前を挿入しています。
こうして組み立てたDOMを呼び出しもとに返しています。

@<list>{js5}の関数は役職のリストと、役職を選ばれた時の挙動を作成しています。
この関数では、画像リストの何番目かとその画像リストの番号に対応したplayerDataリストのアイテムを受け取っています。
また、DOMを作成する関数以外にも役職の名前と表示名のオブジェクトのリストを作成しています。
それでは、関数を見ていきましょう。
ここではselectタグを作成しています。
このタグは文字通りいくつかの選択肢から項目を選ぶ入力欄になっています。
selectタグの中にoptionタグを作り、その中に表示させるテキストを記入することで選択肢を作ることができます。
今回、selectタグにはid属性とvalue属性を付けています。
id属性はそれぞれの選択肢が画像と紐づいていないといけないので、画像のリスト番号を利用しています。
value属性は現在どの選択肢が選ばれているかを示しています。
初期値では村人を選ぶようにするために今回は'murabito'を代入しています。
optionタグは選択肢すべてを書いていたら恐ろしいほど長くなるので、関数外で作成したリストを利用してループで作ります。
optionタグのvalue属性は選択肢として選択したときに、JSで制御するための値です。
無くても動きはしますが、選択する機能を作る場合は必須です。
